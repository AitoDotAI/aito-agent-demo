...
./do dev        # next dev (UI) on :4100 + uvicorn on :4101, hot-reload both
...
- **`/api/*` works in production but 404 in dev** → the dev rewrite in `next.config.ts` proxies to `BACKEND_PORT` (default 4101); make sure your backend is on the matching port.
...
- [docs/v2-migration.md](./docs/v2-migration.md) — the `/api/v2` migration: how to
  ...
  and the core gaps that still block the cutover. **Prod is still v1** — the two
  env vars default to v1/`env.master`, so don't "helpfully" flip them.
